...
class WigglerFieldFitter:
    ...
    def select_xy(self):
        ...
        for field in ["Bx", "By", "Bs"]:
            ...
            self.fit_der2[field] = np.zeros_like(self.raw_data[field])
    ...
    def _fit_transverse_polynomials(self, points=[-1, 0, 1], degree=2):
        ...
        for field in ["Bx", "By", "Bs"]:
            ...
            if degree >= 2:
                ...
                self.raw_der2[field] = 2 * polys[field][:, -(2 + 1)]
            else:
                self.raw_der2[field] = polys[field][:, -1]
...
class WigglerFull:
    def __init__(self, WigglerFieldFitter, WigglerFieldFitter_Der):
        ...
        self.WigglerFieldFitter_Der = WigglerFieldFitter_Der

    def set_segments(self):
        curvature = 0.0
        a1_segments = self.WigglerFieldFitter.export_piecewise_segments("Bx")
        a3_segments = self.WigglerFieldFitter_Der.export_piecewise_segments("Bx")
        b1_segments = self.WigglerFieldFitter.export_piecewise_segments("By")
        b3_segments = self.WigglerFieldFitter_Der.export_piecewise_segments("By")
        bs_segments = self.WigglerFieldFitter.export_piecewise_segments("Bs")
        ...
        a1_start = a1_segments[0][0]
        a3_start = a3_segments[0][0]
        b1_start = b1_segments[0][0]
        b3_start = b3_segments[0][0]
        bs_start = bs_segments[0][0]

        a1_end = a1_segments[0][1]
        a3_end = a3_segments[0][1]
        b1_end = b1_segments[0][1]
        b3_end = b3_segments[0][1]
        bs_end = bs_segments[0][1]

        field_segments = [
            WigglerSegment(start, end, curv=curvature, a=a_i, b=b_i, bs=expr)
