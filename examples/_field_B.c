...
void evaluate_B(const double x_array[], const double y_array[], const double s_array[], size_t n, const double **params, double Bx_out[], double By_out[], double Bs_out[]){
	for (size_t ii = 0; ii < n; ++ii) {
		...
		const double x4 = b_3_0 + b_3_1*s + b_3_2*x0 + b_3_3*x1 + b_3_4*x2;
		const double x5 = x*y;
		const double x6 = (1.0/2.0)*x*x;
		const double x7 = 2*s;
		const double x8 = 3*x0;
		const double x9 = 4*x1;

		// Reduced expressions
		Bx_out[ii] = a_1_0 + a_1_1*s + a_1_2*x0 + a_1_3*x1 + a_1_4*x2 + x*(a_2_0 + a_2_1*s + a_2_2*x0 + a_2_3*x1 + a_2_4*x2) + x3*y + x4*x5 + x6*(a_3_0 + a_3_1*s + a_3_2*x0 + a_3_3*x1 + a_3_4*x2);
		By_out[ii] = b_1_0 + b_1_1*s + b_1_2*x0 + b_1_3*x1 + b_1_4*x2 + x*x3 + x4*x6;
		Bs_out[ii] = bs_0 + bs_1*s + bs_2*x0 + bs_3*x1 + bs_4*x2 + (1.0/6.0)*x*x*x*(a_3_1 + a_3_2*x7 + a_3_3*x8 + a_3_4*x9) + x*(a_1_1 + a_1_2*x7 + a_1_3*x8 + a_1_4*x9) + x5*(b_2_1 + b_2_2*x7 + b_2_3*x8 + b_2_4*x9) + x6*y*(b_3_1 + b_3_2*x7 + b_3_3*x8 + b_3_4*x9) + x6*(a_2_1 + a_2_2*x7 + a_2_3*x8 + a_2_4*x9) + y*(b_1_1 + b_1_2*x7 + b_1_3*x8 + b_1_4*x9);
}
}
...
/*  0 */ _CFFI_OP(_CFFI_OP_FUNCTION, 11), // void()(double const *, double const *, double const *, size_t, double const * *, double *, double *, double *)
/*  1 */ _CFFI_OP(_CFFI_OP_POINTER, 10), // double const *
/*  2 */ _CFFI_OP(_CFFI_OP_NOOP, 1),
/*  3 */ _CFFI_OP(_CFFI_OP_NOOP, 1),
/*  4 */ _CFFI_OP(_CFFI_OP_PRIMITIVE, 28), // size_t
/*  5 */ _CFFI_OP(_CFFI_OP_POINTER, 1), // double const * *
/*  6 */ _CFFI_OP(_CFFI_OP_POINTER, 10), // double *
...
static void _cffi_d_evaluate_B(double const * x0, double const * x1, double const * x2, size_t x3, double const * * x4, double * x5, double * x6, double * x7)
{
  evaluate_B(x0, x1, x2, x3, x4, x5, x6, x7);
}
#ifndef PYPY_VERSION
static PyObject *
_cffi_f_evaluate_B(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_UnpackTuple(args, "evaluate_B", 8, 8, &arg0, &arg1, &arg2, &arg3, &arg4, &arg5, &arg6, &arg7))
    return NULL;
  ...
  if (datasize != 0) {
    ...
    if (_cffi_convert_array_argument(_cffi_type(6), arg7, (char **)&x7,
            datasize, &large_args_free) < 0)
      return NULL;
  }
...
#else
#  define _cffi_f_evaluate_B _cffi_d_evaluate_B
#endif
...
#ifdef PYPY_VERSION
PyMODINIT_FUNC
_cffi_pypyinit__field_B(const void *p[])
{
...
#  ifdef _MSC_VER
     PyMODINIT_FUNC
#  if PY_MAJOR_VERSION >= 3
     PyInit__field_B(void) { return NULL; }
#  else
     init_field_B(void) { }
#  endif
#  endif
...
PyMODINIT_FUNC
PyInit__field_B(void)
{
  return _cffi_init("_field_B", 0x2601, &_cffi_type_context);
}
#else
PyMODINIT_FUNC
init_field_B(void)
{
  _cffi_init("_field_B", 0x2601, &_cffi_type_context);
}
#endif
