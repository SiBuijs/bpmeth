...
def write_to_c(max_order=multipole_order, poly_order=4, field='B'):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(
            f"void evaluate_{field}(const double x_array[], const double y_array[], const double s_array[], size_t n, const double **params, const int multipole_order, double {field}x_out[], double {field}y_out[], double {field}s_out[]){{\n\n")
        ...
        for order in range(1, multipole_order+1):
            ...
            for order, expr in enumerate(reduced_exprs):
                f.write(f"\t\t\t{names[order]}[ii] = {printer.doprint(expr)};\n")

            f.write(f"\t\t}}\n")
            f.write(f"\t\treturn;\n\n")
        f.write(f"\tdefault:\n")
        f.write("\t\tprintf(\"Error: Unsupported multipole order %d\\n\", multipole_order);\n")
        f.write(f"\t\tprintf(\"Supported orders are 0 to {multipole_order - 1}\\n\");\n")
        f.write(f"\t\tprintf(\"Setting field values to zero.\\n\");\n")
        f.write("\t\tfor (size_t ii = 0; ii < n; ++ii) {\n")
        f.write("\t\t\t// Reduced expressions\n")
        for order in range(3):
            f.write(f"\t\t\t{names[order]}[ii] = 0;\n")
        f.write(f"\t\t}}\n")
        f.write(f"\t\treturn;\n")
